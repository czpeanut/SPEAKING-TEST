...
- **後端**：Node.js + Express。收到照片與文字後：呼叫 Gemini TTS 產生語音檔、把照片和語音檔都暫存並提供可公開存取的網址（D-ID 不接受 data URL，只吃真的網址）、把這兩個網址交給 D-ID 建立影片工作、輪詢直到完成，回傳影片網址給前端下載/播放。金鑰全部留在伺服器端。
...
3. 一組 [D-ID API key](https://www.d-id.com)，**而且帳號要實際開通方案或試用額度**——光是註冊拿到 key 不夠，額度是 0 的話每次生成都會收到「not enough credits」，要去 D-ID 後台加值/開通才會真的有額度可以用
4. 一張正臉照片，**JPEG 或 PNG**（D-ID 不接受 WEBP）
...
**注意**：D-ID 需要能連到你伺服器暫存的照片和語音檔網址，而且明確要求是 **https**。本機用 `http://localhost` 跑的話 D-ID 連不進來也不接受非 https 網址，只有部署到有公開 https 網址的環境（如 Render，會自動有 https）才能真的產生影片；本機主要用來檢查介面和其他邏輯（輸入驗證、Gemini TTS 呼叫本身都能在本機測）。
...
- 綁定 Gemini、D-ID 兩家供應商；D-ID 是付費服務，按生成的影片秒數（額度）計費，帳號要先加值/開通方案才有額度
- 需要清晰的正臉照片，且只接受 JPEG/PNG
- 文件內容上限 1500 字，超過會被截斷
- 整個流程（語音合成 + D-ID 生成）約需 1-2 分鐘，視文字長度而定
- 暫存的照片和語音檔網址只在生成期間存在，生成完成或 15 分鐘後會被清除
